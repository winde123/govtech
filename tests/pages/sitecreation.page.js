...
class SiteCreationForm {
  ...
  constructor(page) {
    ...
    this.site_url_input_field = page.getByRole("textbox", {
      name: "e.g. yourgowhereurl",
    });
    this.style_selector = page.getByText(
      "Choose a styleYou will be still able to change it later.Deep oceanFuji"
    );
    this.style_choice_deep_ocean = page.locator('label').filter({ hasText: 'Deep ocean' });
    this.style_choice_fuji_apple = page.locator('label').filter({ hasText: 'Fuji Apple' });
    ...
    this.err_msg_site_url_input_field = page.getByTestId('form-field-group-error-message')
  }
  async select_style(selection) {
    const style_selector = selection;
    await this.page.waitForSelector('label.sc-gHJCvS.fPdMNC',{state:'visible'})
    await this.page.locator('label').filter({ hasText: `${style_selector}` }).click();
